...
function uninstall(cusa, type) {
  $.get("/uninstall/" + type + "/" + cusa, (req,res) => {
    ...
    :((location.href="#alerteDisplay"),setInnerHTML(a,'<div id="alerte" class="alert alert-danger" role="alert">'+rpi[0][defaultLangue].error+"</div>"));
    hideAlert();
  });
}
  ...
  setTimeout(function(){
    $("#titleid-list").append(
      '<div class="col-4">' +
        ...
        '<div class="row">' +
        '<div class="col-4">' +
        '<p id="meta-cusa-'+npTitleId+'" class="meta-p"></p></div>' +
        '<div class="col-2">' +
        '<p id="meta-psvr-'+npTitleId+'" class="meta-p"></p></div>' +
        '<div class="col-2">' +
        '<p id="meta-rev-'+npTitleId+'" class="meta-p"></p></div>' +
        '<div class="col-2">' +
        '<p id="meta-cat-'+npTitleId+'" class="meta-p"></p></div>' +
        "</div>" +
        '<div class="row">' +
        '<div class="col-12">' +
...
function getMetaJson(url, datas) {
  ...
  } else buildCard(datas,datas.size);
}
function makeCard(datas) {
  ...
  else {let data={npTitleId: datas.cusa,icons:[{icon:"assets/img/nopsn.png"}],names:[{name:datas.file}],file:datas.file,};
...
function buildRPI() {
  ...
  let buildRpi =
    ...
    '<div class="form-group mx-sm-2 mb-2">' +
    '<input type="text" name="input-task" class="form-control" placeholder="TitleId" value=""></div>' +
